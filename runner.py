...
def looks_like_libur(info: dict) -> bool:
    # kalau jadwal tidak mengandung jam sama sekali => libur / tidak terjadwal
    ...
    # kata-kata umum libur/non jadwal
    blob = " ".join([
        (info.get("today") or ""),
        jmasuk,
        jpulang,
...
def parse_window_from_infoday(info: dict) -> tuple[str, str, str, str] | None:
    # ambil 2 jam pertama dari string jadwal
...
# ================== Logic sukses + filter NTFY ==================
def is_real_success(ok_send: bool, msg: str) -> bool:
    """
    Syarat NTFY hari kerja:
    - ok_send True
...
def fmt_infoday_pretty(info: dict) -> str:
    ...
    lines.append(f"Jadwal : Masuk {jmasuk} | Pulang {jpulang}")
    lines.append(f"Status : Masuk {masuk} | Pulang {pulang}")
    if ket:
...
def run_once(type_name: str):
    ...
    # Kalau sudah marked, kita stop tanpa sndloc
...
def main():
    ...
    try:
        ...
        # ===== Ambil INFODAY sekali untuk ambil window dinamis =====
        # Cara: pakai flow login-auth-infoday ringan, reuse run_once? (biar simple: panggil run_once tapi dengan "MASUK"
        # TAPI run_once bisa kirim sndloc kalau belum masuk.
        # Jadi kita buat "probe" login-auth-infoday TANPA sndloc:
        ...
        save_cookie_value(extract_ekin_cookie_from_session(session))
        if not ok2:
            print(f"AUTH gagal: {msg2}")
            return

        ok3, info_obj = infoday_raw(session, token, akun)
        save_cookie_value(extract_ekin_cookie_from_session(session))
        if not ok3:
            print(f"INFODAY gagal: {info_obj}")
            return
        ...
        if looks_like_libur(info):
            # notif 1x per hari per akun
            if not was_notified(today, "LIBUR"):
                title = f"DKPP_AUTO {RUN_KEY} LIBUR"
                body = []
                body.append(f"Akun  : {RUN_KEY}")
                body.append(f"Device: {device_name} ({device_id})")
                body.append(f"Waktu : {now.isoformat(sep=' ', timespec='seconds')}")
                body.append("")
                body.append(fmt_infoday_pretty(info))
                ntfy(title, "\n".join(body), priority="default")
                mark_notified(today, "LIBUR")
            print("Hari libur / tidak terjadwal. Exit.")
            return

        win = parse_window_from_infoday(info)
        if not win:
            # dianggap libur/invalid juga (tetap notif 1x/hari)
            if not was_notified(today, "LIBUR"):
                title = f"DKPP_AUTO {RUN_KEY} LIBUR/INVALID"
                body = []
                body.append(f"Akun  : {RUN_KEY}")
                body.append(f"Device: {device_name} ({device_id})")
                body.append(f"Waktu : {now.isoformat(sep=' ', timespec='seconds')}")
                ...
                ntfy(title, "\n".join(body), priority="default")
                mark_notified(today, "LIBUR")
            print("Window tidak valid (jadwal tidak terbaca). Exit.")
            return
        ...
        for tname in tasks:
            ...
            # update last state (biar kelihatan log di state)
            ...
            # DONE + NTFY hari kerja: HANYA jika "Berhasil:" + ada "DINAS" + tidak ada "terkendala/admin"
            if is_real_success(ok_send, msg_send):
                if not was_notified(today, tname):
                    ...
                    body = []
                    body.append(f"Akun  : {RUN_KEY}")
                    body.append(f"Device: {device_name2} ({device_id2})")
                    body.append(f"Action: {tname}")
                    body.append(f"Hasil : {msg_send}")
                    body.append(f"Waktu : {tz_now_wib().isoformat(sep=' ', timespec='seconds')}")
                    body.append("")
                    body.append(fmt_infoday_pretty(info_after))
                    ntfy(title, "\n".join(body), priority="default")
                    mark_notified(today, tname)

                # baru dianggap DONE kalau real success
                mark_done(today, tname, True, msg_send, infoday_obj)
                print(f"{tname}: DONE - {msg_send}")
            else:
                # kalau mengandung terkendala -> tidak ntfy, tidak done (biar retry di run berikutnya)
